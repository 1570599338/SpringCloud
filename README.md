...
#### Cookie与头消息
    1、Spring Cloud Zuul在请求路由时，会过滤掉HTTP请求头部信息中的一些敏感信息，防止他们会被传递到下游的外部服务器。
    默认的敏感的头部信息通过zuul.sensitiveHeaders参数定义，包括Cookie、Set-Cookie、Authorization三个属性
    2、解决办法
        a、通过设置全局参数为空来覆盖默认值
            zuul.senstiveHeaders=    这种方法并不推荐
        b、通过指定路由的参数来配置，方法有2种
            1、对指定路由开启自定义敏感头
                zuul.routes.<router>.customSenstiveHeaders=true
            2、将指定路由的敏感头设置为空
...
#### 重定向问题
 ...
 #### Hystrix 和 Ribbon支持
     ...
     但是需要注意，当使用path与url的映射关系来配置路由规则的时候，对于路由转发的请求不会采用HystrixCommand
     来包装，所以这类路由请求没有线程隔离和断路器的保护，并且也不会有负载均衡的能力。因此，我们在使用Zuul的时候尽量使用
     path和serviceId的组合来进行配置，这样不仅可以保证API网关的健壮和稳定，也能用到Ribbon的客户端负载均衡功能。
 
        
        
        
#### 过滤器详解 
    Zuul第一印象是这样：它包含了对请求的路由和过滤两个功能，
        路由功能：主要负责将外部请求转发到具体的微服务实例上，是实现外部访问统一入口的基础；
        过滤器：而过滤器功能则负责对请求的处理过程进行干预，是实现请求校验、服务聚合等功能的基础。
         
         实际上，路由功能在真正运行时，它的路由印社和请求转发都是由几个不同的过滤器完成的，
                路由映射主要通过pre类型的过滤完成，它将请求路径与配置的路由跪着进行匹配，
                以找到需要转发的目的地址；而请求抓发的部分则是由路由route类型的过滤器来完成，
                对pre类型过滤器获得的路由地址进行转发。
                        
    ZuulFilter过滤器包含4个基本特征:filterType()过滤类型、filterOrder()执行顺序、shouldFilter()执行条件、run()具体操作。    
      filterType（）该函数需要返回一个字符串来代表过滤器的类型，而这个类型就是在HTTP请求过程中定义的各个阶段。
                    在ZUUL中默认定义了4中不同生命周期的过滤器类型，具体如下所示：
                    pre:可以在请求被路由之前调用。
                    routing:在路由请求时被调用。
                    post:在routing和error过滤器之后被调用。
                    error：处理请求时发生错误时被调用。
                    
       filterOrder()：通过int值来定义过滤器的执行顺序，树枝越小优先级越高。
       shouldFilter()：返回boolean值来按段该过滤器是否被执行，我们可以通过此方法来制定过滤器的有效范围。
       run():过滤器的具体逻辑。  
...
### 第8章、分布式配置中心：Spring Cloud config
        作用：用来为分布式系统中的基础设施和微服务应用提供集中化的外部配置支持，它分为服务端与客户端两个部分。
        服务端：也称为分布式诶之中心，它是一个独立的 微服务应用，用来连接配置长裤并为客户端提供获取配置信息、加密/解密信息等访问接口；
        客户端：通过指定的配置中心来管理应用资源与业务相关的配置内容，并在启动时候从配置中心获取和加载配置信息。
        
        1、spring cloud config实现的配置中心默认采用的Git来存储配置信息，所以使用Springf cloud config构建的配置服务器，
            天然就支持对微服务应用信息的版本管理，并通过git客户端工具来方便地管理和访问配置内容。
